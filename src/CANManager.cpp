...
bool CANManager::begin()
{
    ...
    g_config.rx_queue_len = 32;  // headroom for 8 CSCs replying simultaneously
    ...
    // BMW i3 standard mode: send wake frame. Bus-pack (BMWI3BUS) does not
    // use the 0x130 wake sequence — sending it is harmless but unnecessary.
...
void CANManager::rxTaskFn(void *param)
{
    ...
    while (self->running) {
        if (twai_receive(&msg, pdMS_TO_TICKS(100)) == ESP_OK) {
            ...
            // On receive timeout, check for bus-off and do a full driver recovery.
            // twai_initiate_recovery() alone is insufficient — uninstall+reinstall
            // also resets the TX queue, preventing tx_msg_count going negative
            // which would assert-crash the ESP-IDF TWAI driver.
            ...
            if (twai_get_status_info(&status) == ESP_OK &&
                ...
                if (twai_driver_install(&g, &t, &f) == ESP_OK &&
                    twai_start() == ESP_OK) {
                    // Reset module state so keepalive counter restarts cleanly
                    for (int m = 0; m < I3_MAX_MODS; m++)
                        self->i3data[m].lastSeenMs = 0;
                    Logger::warn("CAN: recovered from bus-off");
                } else {
                    Logger::error("CAN: recovery failed, stopping");
                    self->running = false;
                }
            }
        }
...
void CANManager::processRxFrame(const twai_message_t &msg)
{
        ...
        id == 0x305 || id == 0x306)
    {
        ...
        externalDeviceSeen = true;   // charger/inverter present
    ...
    // Any frame outside the BMW CSC range means an external device (inverter/charger/
    // analyser) is on the bus that will ACK our Victron summary frames.
...
void CANManager::sendFrame(uint32_t id, uint8_t *data, uint8_t len)
{
    ...
    // Guard: calling twai_transmit() during bus-off with pending frames causes
    // tx_msg_count to go negative, triggering an assert crash in ESP-IDF.
    twai_status_info_t status;
    if (twai_get_status_info(&status) != ESP_OK ||
    ...
    // Non-blocking: drop frame rather than blocking the loop task if queue full.
